...
public class Mods {
	...
	private void setIDShelfmark(String place, String institution, String shelfmark) {
		if (!place.isEmpty() && !institution.isEmpty() && !shelfmark.isEmpty()) {
			Element ms_identifying_shelfmark = new Element("metadata", kitodo).setAttribute("name","TitleDocMain").setText(place + ", " + institution + ", " + shelfmark);
			intern.addContent(ms_identifying_shelfmark);
		}
	}
	...
	private void setShelfmarkHash() {
		...
		intern.addContent(ms_identifying_shelfmark_hash);
	}

	private void setAttribute(String definition) {
		String xPath = mapping.getProperty(definition);
		xpe = fact.compile(xPath, Filters.attribute(), null, spaceList);
		if (!(xpe.evaluateFirst(docHida) == null)) {
			...
			if (value.contains("&")) {
				...
				if (value.indexOf("&", i) == value.lastIndexOf("&")) {
					...
					Element ms_medium = new Element("metadata", kitodo).setAttribute("name","ms_medium").setText(child);
					intern.addContent(ms_medium);
				} else {
				...
				Element ms_medium = new Element("metadata", kitodo).setAttribute("name","ms_medium").setText(value);
				intern.addContent(ms_medium);
			}
		}
	}
